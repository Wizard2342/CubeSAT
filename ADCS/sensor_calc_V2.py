...
def roll_am(accelX,accelY,accelZ):
    accelX2 = accelX*accelX
    accelY2 = accelY*accelY
    accelZ2 = accelZ*accelZ
    roll = np.arctan(accelY/(math.sqrt(accelX2+accelZ2)))
    return roll
...
def pitch_am(accelX,accelY,accelZ):
    ...
    accelY2 = accelY*accelY
    accelZ2 = accelZ*accelZ
    pitch = np.arctan(accelX/(math.sqrt(accelY2+accelZ2)))
    return pitch

def yaw_am(accelX,accelY,accelZ,magX,magY,magZ):
    roll = roll_am(accelX,accelY,accelZ)
    pitch = pitch_am(accelX,accelY,accelZ)
    magx = magX*np.cos(pitch) + magY*np.sin(roll)*np.sin(pitch) + magZ*np.cos(roll)*np.sin(pitch)
    magy = magY*np.cos(roll) - magZ*np.sin(roll)
    yaw = np.arctan(-magy/magx)
    return yaw
...
def pitch_gy(prev_angle, delT, gyro):
    ...
    if(prev_angle != None):
        ...
        pitch = accel_gyro/mag + gyro*delT
    return pitch
